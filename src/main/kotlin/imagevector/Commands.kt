...
internal val Command.method: String
    get() = "$methodName($methodParams)"
    ...
    get() =
        when (this) {
            ...
            is Command.HorizontalLineTo ->
                if (isAbsolute) "horizontalLineTo" else "horizontalLineToRelative"
            ...
            is Command.ReflectiveCurveTo ->
                if (isAbsolute) "reflectiveCurveTo" else "reflectiveCurveToRelative"
            is Command.VerticalLineTo ->
                if (isAbsolute) "verticalLineTo" else "verticalLineToRelative"
            is Command.QuadraticBezierTo -> if (isAbsolute) "quadTo" else "quadToRelative"
            is Command.ReflectiveQuadraticBezierTo ->
                if (isAbsolute) "reflectiveQuadTo" else "reflectiveQuadToRelative"
...
internal val Command.methodParams: String
    get() =
        when (this) {
            Command.Close -> ""
            is Command.ArcTo ->
                floatParams(horizontalEllipseRadius, verticalEllipseRadius, theta) +
