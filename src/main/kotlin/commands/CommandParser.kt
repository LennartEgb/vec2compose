...
internal class CommandParser {
    ...
    private fun createHorizontalLinesTo(eventString: String, isAbsolute: Boolean): List<Command> {
        return eventString.prepare().validate(1, "Horizontal line").map {
            Command.HorizontalLineTo(it, isAbsolute)
        }
    }

    private fun createVerticalLinesTo(eventString: String, isAbsolute: Boolean): List<Command> {
        return eventString.prepare().validate(1, "Vertical line").map {
            Command.VerticalLineTo(it, isAbsolute)
        }
    }
    ...
    private fun createCurvesTo(eventString: String, isAbsolute: Boolean): List<Command> {
        return eventString
            ...
            .map {
                Command.CurveTo(it[0], it[1], it[2], it[3], it[4], it[5], isAbsolute = isAbsolute)
            }
    }

    private fun String.prepare(): List<Float> =
        valueRegex.findAll(this).map { it.value }.map(String::toFloat).toList()
