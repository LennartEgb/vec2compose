...
internal class VectorDrawableColorParser(
    private val colorParser: HexColorParser = HexColorParser()
) {

    private companion object {
        const val ANDROID_COLOR_PREFIX = "@android:color/"
    }

    fun parse(value: String): ImageVector.Path.FillColor? {
        return if (value.startsWith(ANDROID_COLOR_PREFIX)) {
            androidColorsMap[value.substringAfter(ANDROID_COLOR_PREFIX)]
                ?.let { hex -> colorParser.parse(color = hex) }
        } else {
